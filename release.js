...
class ConcatPlugin {
  constructor(options) {
    ...
    this.settings = options; // used to determine if we should emit files during compiler emit event
  ...
  getRelativePathAsync(context) {
    return Promise.all(this.settings.filesToConcat.map(f => {
      ...
      return f;
    })).then(rawResult => rawResult.reduce((target, resource) => target.concat(resource), [])).catch(error => {
      console.error(error);
    });
  }
  /**
  ...
  resolveReadFiles(compiler) {
    ...
    this.filesToConcatAbsolutePromise = new Promise((resolve, reject) => {
      compiler.resolverFactory.hooks.resolver.for('normal').tap('resolver', resolver => {
        resolve(relativePathArrayPromise.then(relativeFilePathArray => Promise.all(relativeFilePathArray.map(relativeFilePath => new Promise((_resolve, _reject) => resolver.resolve({}, compiler.options.context, relativeFilePath, {}, (error, filePath) => {
          if (error) {
    ...
    const createNewPromise = () => {
      ...
      return this.filesToConcatAbsolutePromise.then(filePathArray => Promise.all(filePathArray.map(filePath => new Promise((resolve, reject) => {
  ...
  apply(compiler) {
    ...
    const dependenciesChanged = (compilation, filesToConcatAbsolute) => {
      ...
      const changedFiles = fileTimestampsKeys.filter(file => {
